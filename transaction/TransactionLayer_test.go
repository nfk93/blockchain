...
func TestReceiveBlock(t *testing.T) {
	...
	b1 := createBlock([]Transaction{t1, t2}, 1, p1, sk1)
	b1.SignBlock(sk1)
	if !tl.ReceiveBlock(b1) {
		t.Error("Block is not accepted at TransactionLayer...")
	}
...
func TestFinalizeBlock(t *testing.T) {
	...
	state := tl.FinalizeBlock(b1.CalculateBlockHash())
	if state.Ledger[p2] != 500 {
		t.Error("Finalization failed or not executed correctly! Stake expected to be 500, but is ", state.Ledger[p2])
...
func TestBlockCreation(t *testing.T) {
	...
	newData := CreateBlockData{[]Transaction{t1, t2}, p1, b1.Slot + 1, ""}
	newBlock := tl.CreateNewBlock(newData)

	if newBlock.ParentPointer != b1.CalculateBlockHash() {
		t.Error("Something went wrong with the Block Creation")
	}
...
func createBlock(t []Transaction, i int, pk PublicKey, sk SecretKey) Block {

	nonce := BlockNonce{"42", "", pk}
...
func createTl(sk SecretKey) TransLayer {
	tl := StartTransactionLayer(sk, State{})

	return tl
}

//func startTreeInTL(tl *TransLayer)  {
//	tl.ReceiveBlock()
//	sk, pk := KeyGen(2048)
//	t1 := CreateTransaction(p1, p1, 200, strconv.Itoa(1), sk1)
//	block1 := createBlock([]Transaction{t1}, 0, p1)
